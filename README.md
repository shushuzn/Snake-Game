...
- **9 种模式**：经典、限时 60 秒、冲刺 45 秒、无尽关卡、肉鸽模式、AI对战（简单/普通/困难/地狱难度）、多人对战（2-4人）、观战模式、每日挑战。
...
- v1.42.0：全息启动界面 - 开局遮罩从朴素文字升级为全息启动卡：霓虹大标题「🐍 SNAKE //协议」（青紫发光 + 脉冲）、标语「深空贪吃蛇 · 连击 · 道具 · 多模式」、脉冲提示「▶ 按方向键开始游戏」+ 灰色控制说明；同步更新 index.html 初始结构与 game.js 的 showStartOverlay（避免模块就绪后被覆写）。
- v1.41.0：连击系统增强 - 修复平衡 bug(combo 硬上限 9 使 combo15 成就永远不可达,上限提至 15);新增里程碑 x12(+20 分)/x15(满连击 +30 分);连击 ≥3 断连时红色闪烁提示(此前断连无任何反馈)。

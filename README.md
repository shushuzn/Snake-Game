...
- **进行中（v0.89）**
  - 🔄 远端榜接口真实接入（读接口优先，失败自动降级本地）。
- **下一步（v0.91）**
  - ⏭ 榜单分维度扩展（模式榜 / 每日挑战榜 / DLC 分类榜）。
...
### 版本推进计划（重排）
- `v0.88`：活动规则包扩展（周末/节日）+ 活动文案/倍率说明统一。
- `v0.89`：排行榜远端接口真实接入（读链路 + 降级回退）。
- `v0.90`：活动入口聚合 + 地图摘要信息首版。
- `v0.91`：榜单分维度扩展 + 地图分享校验增强。
- `v0.92`：复盘建议路径提示 + 新手引导分层首版。

### 路线图执行看板（滚动）
- **Now（进行中）**：`v0.88` 活动规则包扩展（周末/节日）。
- **Next（下一步）**：`v0.89` 排行榜远端接口真实接入。
- **Later（随后）**：`v0.90` 活动入口聚合 + 地图摘要信息。

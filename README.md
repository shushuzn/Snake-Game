...
```bash
python3 skills/snake-feature-evolver/scripts/bump_version.py 0.94.0
```
...
- `v0.95`：分享前校验增强（超限障碍、重叠点、边界质量提示）。
- `v0.96`：复盘建议路径提示 + 新手引导分层首版。

### 路线图执行看板（滚动）
- **Now（进行中）**：`v0.95` 分享前校验增强（超限障碍、重叠点、边界质量提示）。
- **Next（下一步）**：`v0.96` 复盘建议路径提示 + 新手引导分层。
- **Later（随后）**：`v0.97` 新手引导分层与地图分享质量校验。

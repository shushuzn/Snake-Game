...
```bash
python3 skills/snake-feature-evolver/scripts/bump_version.py 0.81.0
```
...
- `v0.82`：风险收益型 DLC 第二阶段与可视化对比。
- `v0.83`：数据回放与复盘（关键帧摘要 + 死亡原因聚类）。


### 路线图执行看板（滚动）

- **Now（进行中）**：风险收益型 DLC 第二阶段与可视化对比。
- **Next（下一步）**：数据回放与复盘（关键帧摘要 + 死亡原因聚类）。
- **Later（随后）**：活动化运营能力（节日主题挑战包）。

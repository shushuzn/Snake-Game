...
```bash
python3 skills/snake-feature-evolver/scripts/bump_version.py 0.80.0
```
...
- `v0.81`：移动端引导与手势自定义第一版。
- `v0.82`：风险收益型 DLC 第二阶段与可视化对比。


### 路线图执行看板（滚动）

- **Now（进行中）**：移动端引导与手势自定义第一版。
- **Next（下一步）**：风险收益型 DLC 第二阶段与可视化对比。
- **Later（随后）**：数据回放与复盘（关键帧摘要 + 死亡原因聚类）。

...
```bash
python3 skills/snake-feature-evolver/scripts/bump_version.py 0.83.0
```
...
- `v0.84`：活动化运营能力（节日主题挑战包）。
- `v0.85`：排行榜远端接口切换开关。


### 路线图执行看板（滚动）

- **Now（进行中）**：活动化运营能力（节日主题挑战包）。
- **Next（下一步）**：排行榜远端接口切换开关。
- **Later（随后）**：数据回放增强（关键帧时间线）。

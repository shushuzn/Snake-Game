...
```bash
python3 skills/snake-feature-evolver/scripts/bump_version.py 0.93.0
```
...
- `v0.93`：✅ 活动规则可配置化（已完成）。
- `v0.94`：榜单继续扩展（每日挑战榜 / DLC 分类榜）+ 分享前校验增强。
- `v0.95`：复盘建议路径提示 + 新手引导分层首版。

### 路线图执行看板（滚动）
- **Now（进行中）**：`v0.94` 榜单继续扩展（每日挑战榜 / DLC 分类榜）+ 分享前校验增强。
- **Next（下一步）**：`v0.95` 复盘建议路径提示 + 新手引导分层。
- **Later（随后）**：`v0.96` 新手引导分层与地图分享质量校验。

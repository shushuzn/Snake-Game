...
```bash
python3 skills/snake-feature-evolver/scripts/bump_version.py 0.87.0
```
...
> 目标：在保持“纯前端、零依赖可运行”的前提下，优先提升**可复玩性**、**可分享性**和**移动端体验**。

### 已完成里程碑（P0-P1）
- ✅ 稳定每日挑战与 DLC 共存逻辑（跨天/重置场景一致）。
- ✅ 结算系统细化（加时来源 + 得分来源拆分）。
- ✅ 设置与创意工坊字段演进（`schemaVersion` + 迁移补齐）。
- ✅ `game.js` 主流程模块化拆分收敛（挑战、存档、计时、状态机等模块落地）。
- ✅ DLC 风险/收益可视化、周循环主题、跨天一致性与挑战链路去冗余优化。

### P2（下一阶段，3-6 周）
1. **排行榜与赛季系统（可选后端）**
   - 范围：模式榜、每日挑战榜、DLC 分类榜、赛季重置与赛季奖励展示。
   - 已完成：
     - ✅ 本地排行榜 Top20 面板（按分数/时间排序 + 本地持久化）。
     - ✅ 赛季信息区与历史赛季入口（最近6期 + 月赛季剩余时间）。
   - 交付：
     - 前端榜单面板（支持本地模拟榜 + 远端接口切换开关）。
     - 赛季信息区（赛季剩余时间、赛季最佳、历史赛季入口）。
   - 验收标准：
     - 列表渲染可在 200 条数据以内稳定滚动；
     - 离线时自动回退本地榜并给出状态提示。
...
3. **移动端交互与新手引导增强**
   - 范围：触屏手势自定义、分层教程、动态提示。
   - 已完成：
     - ✅ 滑动灵敏度设置（12/18/24/32px）与本地持久化。
   - 交付：
     - 新手引导流程（基础操作 -> 道具认知 -> 模式策略）；
     - 手势灵敏度与方向判定阈值设置。
   - 验收标准：
     - 移动端 390px 宽度下核心 HUD 不遮挡棋盘；
     - 新手引导可中断并续看。

### P3（中长期，6-10 周）
1. **玩法深度扩展**
   - ✅ 风险收益型 DLC 第二阶段（额外副作用/组合收益）与可视化对比。
2. **数据回放与复盘**
   - ✅ 最近一局复盘摘要面板（终局原因/关键指标）已上线；后续补充关键帧时间线与建议路径提示。
3. **活动化运营能力（纯前端可降级）**
   - ✅ 节日主题挑战包首版已上线；✅ 赛季奖励已支持与活动加成联动；后续补充周末活动规则包与版本活动入口。

### 当前建议执行顺序（建议按版本推进）
- `v0.77 ~ v0.78`：地图码（编码/校验/导入导出）+ 工坊互通。
- `v0.79`：✅ 榜单面板前端实现 + 本地模拟榜（已完成）。
- `v0.80`：✅ 赛季信息区与历史赛季入口（已完成）。
- `v0.81`：✅ 移动端交互增强第一版（滑动灵敏度可配置）已完成。
- `v0.82`：✅ 风险收益型 DLC 第二阶段与可视化对比（已完成）。
- `v0.83`：✅ 数据回放与复盘首版（最近一局摘要）已完成。
- `v0.84`：✅ 活动化运营能力首版（节日主题挑战包）已完成。
- `v0.85`：✅ 排行榜远端接口切换开关（占位 + 回退提示）已完成。
- `v0.86`：✅ 数据回放增强（关键帧时间线）已完成。
- `v0.87`：✅ 赛季奖励与活动联动展示（已完成）。
- `v0.88`：活动规则包（周末/节日）扩展。
- `v0.89`：排行榜远端接口真实接入。


### 路线图执行看板（滚动）

- **Now（进行中）**：活动规则包（周末/节日）扩展。
- **Next（下一步）**：排行榜远端接口真实接入。
- **Later（随后）**：数据回放建议路径提示。

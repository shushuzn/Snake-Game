...
```bash
python3 skills/snake-feature-evolver/scripts/bump_version.py 0.51.0
```
...
### P0（当前迭代，1-2 周）
- 稳定每日挑战与 DLC 共存逻辑：补齐强制模式/锁定开关在“进行中对局、跨天刷新、切账号”场景下的一致性回归清单。
- 为 `blitz` / 限时类模式增加更细粒度结算面板（加时来源、倍率贡献、连击贡献）。
- 统一创意工坊与本地设置字段演进策略（含版本迁移与旧码兼容）。

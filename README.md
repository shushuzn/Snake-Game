...
```bash
python3 skills/snake-feature-evolver/scripts/bump_version.py 0.89.0
```
...
### 路线图执行看板（滚动）
- **Now（进行中）**：`v0.90` 活动入口聚合 + 地图摘要信息。
- **Next（下一步）**：`v0.91` 榜单分维度扩展 + 活动规则可配置化。
- **Later（随后）**：`v0.92` 复盘建议路径提示 + 新手引导分层。

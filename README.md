...
```bash
python3 skills/snake-feature-evolver/scripts/bump_version.py 0.82.0
```
...
- `v0.83`：数据回放与复盘（关键帧摘要 + 死亡原因聚类）。
- `v0.84`：活动化运营能力（节日主题挑战包）。


### 路线图执行看板（滚动）

- **Now（进行中）**：数据回放与复盘（关键帧摘要 + 死亡原因聚类）。
- **Next（下一步）**：活动化运营能力（节日主题挑战包）。
- **Later（随后）**：排行榜远端接口切换开关。

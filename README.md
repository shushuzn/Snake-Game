...
- **系统机制**：障碍、硬核模式、连击、任务、成就、图鉴、高对比显示开关、精简HUD开关、失焦自动暂停开关、创意工坊规则分享、DLC 扩展包（狂热/守护），以及障碍编辑器（坐标导入/导出）。
- **局外成长**：肉鸽词条 + 肉鸽点成长。
...
```bash
python3 skills/snake-feature-evolver/scripts/bump_version.py 0.50.0
```
